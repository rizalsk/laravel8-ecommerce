...
{{-- <x-user-layout> --}}
    ...
    {{-- <div class="flex justify-center py-10 ">
        <div class="block p-6 w-50 rounded-lg shadow-xl bg-white max-w-sm">
            <x-jet-validation-errors class="mb-4" />

            @if (session('status'))
            <div class="mb-4 font-lg text-xl text-green-600">
                {{ session('status') }}
            </div>
            @endif
            ...
            <form method="POST" action="{{ route('login') }}">
                ...
                <div>
                    <x-jet-label class="text-xl font-bold" for="email" value="{{ __('Email') }}" />
                    <x-jet-input id="email" class="block mt-1 py-2 text-xl w-full" type="email" name="email" :value="old('email')" required autofocus />
                </div>

                <div class="mt-4">
                    <x-jet-label class="text-xl font-bold" for="password" value="{{ __('Password') }}" />
                    <x-jet-input id="password" class="block mt-1 py-2 text-xl w-full" type="password" name="password" required autocomplete="current-password" />
                </div>

                <div class="block mt-4">
                    <label for="remember_me" class="flex items-center text-xl">
                ...
                <div class="flex items-center justify-end mt-4">
                    ...
                    <x-jet-button class="ml-4 text-xl">
                        {{ __('Log in') }}
                    </x-jet-button>
                </div>
            </form>
